        ...
        if ($request->file('foto')) {
            ...
            if ($validatedData['nama'] != $dosen->nama) {
                ...
                if (file_exists($path)) {
                    ...
                    if (!file_exists($pathFotoBaru)) {
                        if (rename($pathFotoLama, $pathFotoBaru)) {
                            $validatedData['foto'] = 'fotodosen/' . $namaFileBaru;
                        } else {
                            return back()->with('Failed', 'Error renaming file');
                        }
                    } else {
                        return back()->with('Failed', 'File with new name already exists');
                    }
                }
            } else {
